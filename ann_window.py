...
def normalize(x):
    ...
    l = [normalize_(i) for i in x]
    return np.array(l)

def normalize_(x):
    x = view_as_windows(x,50)
    scaler = StandardScaler()
    x = scaler.fit_transform(x)
    a,b = x.shape
    return x[:,49:].flatten() #Keeping only the last normalized term of each window
...
def get_supervision(data):
    ...
    for i in data[1:]: #shifting 1 period for prediction
        if(i<q1):
            ...
            l.append([0,0,1])
    l.append([0,1,0]) #2d quantile arbitrarily attributed to the last supervisory vector
    return np.array(l)

#Splitting of dataset into training (x_a) and validation (x_v) sets
...
plt.suptitle('3-class classification, 50-periods normalization', y=0.98, fontsize=14)
